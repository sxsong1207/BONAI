...
class Evaluation:
    ...
    def offset_error_vector(self, title="demo", show_polar=False):
        ...
        if self.show:
            ...
            plt.savefig(
                os.path.join(
                    self.output_dir, "{}_offset_error_polar_evaluation.{}".format(title, self.out_file_format)
                ),
                bbox_inches="tight",
                dpi=600,
                pad_inches=0.1,
            ...
            plt.savefig(
                os.path.join(
                    self.output_dir, "{}_offset_error_length_hist_evaluation.{}".format(title, self.out_file_format)
                ),
                bbox_inches="tight",
                dpi=600,
                pad_inches=0.1,
            ...
            plt.savefig(
                os.path.join(
                    self.output_dir, "{}_offset_error_angle_hist_evaluation.{}".format(title, self.out_file_format)
                ),
                bbox_inches="tight",
                dpi=600,
                pad_inches=0.1,
    ...
    def get_confusion_matrix_indexes_json_gt(self, mask_type="footprint"):
        ...
        for ori_image_name in self.ori_image_name_list:
            ...
            gt_polygons = [
                bstool.mask2polygon(gt_building["footprint_mask"]).buffer(0) for gt_building in gt_buildings
            ]
            pred_polygons = [pred_building["polygon"] for pred_building in pred_buildings]
    ...
    def visualization_boundary(
        ...
        image_dir,
        vis_dir,
        mask_types=["roof", "footprint"],
        with_iou=False,
        with_gt=True,
        with_only_pred=False,
        with_image=True,
    ):
        colors = {"gt_TP": (0, 255, 0), "pred_TP": (255, 255, 0), "FP": (0, 255, 255), "FN": (255, 0, 0)}
        for mask_type in mask_types:
            objects = self.get_confusion_matrix_indexes(mask_type=mask_type)
            ...
            for image_name in os.listdir(image_dir):
                ...
                if (image_basename not in objects) and with_gt:
                    continue
    ...
    def visualization_offset(self, image_dir, vis_dir, with_footprint=True):
        ...
        if with_footprint:
            image_dir = os.path.join(vis_dir, "..", "boundary_pred", "footprint")
        ...
        for image_name in os.listdir(image_dir):
            ...
            for gt_polygon, gt_offset, pred_polygon, pred_offset, gt_height in zip(
                ...
                pred_footprint_centroid = [
                    coordinate - offset for coordinate, offset in zip(pred_roof_centroid, pred_offset)
                ]
...
if __name__ == "__main__":
    ...
    # not used
    output_dir = f"./data/output/{eval_parameters.model}/"
    ...
    if eval_parameters.with_only_vis is False:
        ...
        if evaluation.dump_result:
            ...
            meta_info = dict(
                summary_file=eval_parameters.summary_file,
                model=eval_parameters.model,
                anno_file=eval_parameters.anno_file,
                gt_roof_csv_file=eval_parameters.gt_roof_csv_file,
                gt_footprint_csv_file=eval_parameters.gt_footprint_csv_file,
            ...
            result_dict = {
                "Roof F1: ": segmentation_eval_results["roof"]["F1_score"],
        ...
        if eval_parameters.with_vis:
            ...
            evaluation.visualization_boundary(
                image_dir=eval_parameters.test_image_dir, vis_dir=eval_parameters.vis_boundary_dir, with_gt=True
            )
